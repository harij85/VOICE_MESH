...
void main() {
  ...
  // BUG-008 / BUG-013 FIX: The original code had an early-return for the miss
  // case (t <= 0.0) followed by a redundant `if (t > 0.0)` guard — always
  // true at that point — wrapping the hit shading code.  Between the two
  // branches a background colour was computed but immediately overwritten by
  // the hit block, making it dead code.  Remove both the redundant guard and
  // the dead background initialisation.
  if (t <= 0.0) {
